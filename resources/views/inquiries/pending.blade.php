...
<div style="display: flex;justify-content: right;align-items: right;">
    <form method="POST" action="{{url('print')}}" class="form-inline my-2 my-lg-0">
        @csrf

      <input class="form-control mr-sm-2" name="code" type="search" id="myInput" placeholder="Search To Print" aria-label="Search">
...
<div >
    <a class="btn btn-primary" href="{{url('print')}}">Print</a>
</div>
<div class="continer">
        ...
        <tbody>
           @foreach($records as $inquiry)
            <tr data-id="{{ $inquiry->id }}">
                ...
                <td id="code" >{{$inquiry->code}}</td>

                <td id="code">{{$inquiry->code2}}</td>


                <td style="width: 500px">{{$inquiry->address}}</td>

                <td style="width: 500px">{{$inquiry->address2}}</td>


                <td>{{$inquiry->city}}</td>
                <td>
                @if (app()->getLocale() == 'ar')
                    {{ $inquiry->status == 'Pending' ? __('view.Pending') : $inquiry->status }}
                @else
                    {{ $inquiry->status }}
                @endif
                </td>
